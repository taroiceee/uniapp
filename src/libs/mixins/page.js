export default {
  ...
  methods: {
    ...
    get_obj_main: function get_obj_main(query, func) {
      ...
      this.$get(this.toUrl(query, url), null, function (json, status) {
        const res = JSON.parse(JSON.stringify(json.result));
        if (res === 0 || res) {
          var obj;
          if (res.obj) {
            obj = res.obj;
          ...
          if (obj) {
            ...
            var o = _this.obj;
            for (var k in o) {
              if (k != 'limit_times' && k.indexOf('limit_times') == -1 && k.indexOf('time') !== -1) {
                var val = o[k];
                if (val) {
            ...
            if (!_this.form || Object.keys(_this.form).length === 0) {
              ...
              // console.log('查询一条数据(主程序)', _this.form);
              if (Object.keys(_this.form).length > 0) {
                uni.push(_this.form, form);
              }
            }
  ...
  onShow() {
    ...
    this.user_group = this.userGroup;
