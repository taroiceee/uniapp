    ...
    <view class="tn-custom-nav-bar__bar" :class="[barClass]" :style="[barStyle]">
      <view v-if="isBack">
        ...
        <view v-else class="tn-custom-nav-bar__bar__action" @tap="handlerBack">
          <text
            class="tn-custom-nav-bar__bar__action--nav-back"
            :class="[`tn-icon-${backIcon}`]"
          ></text>
          <text class="tn-custom-nav-bar__bar__action--nav-back-text" v-if="backTitle">{{
            backTitle
          }}</text>
        </view>
      </view>
...
export default {
  ...
  computed: {
    ...
    contentStyle() {
      ...
      style.height =
        this.height === 0
          ? this.customBarHeight - this.statusBarHeight + this.unit
  ...
  methods: {
    ...
    async updateNavBarInfo() {
      ...
      let safeAreaBottom = this.vuex_safe_area_bottom;
      // 如果获取失败则重新获取
      if (!customBarHeight) {
        try {
          ...
          safeAreaBottom = navBarInfo.safeAreaBottom;
        } catch (e) {
          setTimeout(() => {
            this.updateNavBarInfo();
          }, 10);
      ...
      // 更新vuex中的导航栏信息
      this && this.$u.vuex('vuex_status_bar_height', statusBarHeight);
      this && this.$u.vuex('vuex_custom_bar_height', customBarHeight);
      this && this.$u.vuex('vuex_safe_area_bottom', safeAreaBottom);
    ...
    navBack() {
	  // 如果定义了上页地址，则直接跳转至该页面
	  if(this.backUrl) {
	  	if(this.backUrl == '/pages/user/index'){
	  		uni.switchTab({
	  			url: this.backUrl
	  		})
	  		return
