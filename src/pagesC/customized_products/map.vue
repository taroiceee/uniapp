...
<script>
	...
	export default {
		...
		methods: {
      async get_list() {
        let res = await this.$get('~/api/customized_products/get_list',{
          page: 1,
          size: 9999,
        });
        this.markers = res.result.list.map((item) => {
          return {
            id: item.customized_products_id,
            width: 30,
            height: 30,
            ...
            callout: {
              content: ''
                            	                            	                + '商品名称：' + item.name_of_commodity + '\n'
                                            	                + '分类名称：' + item.category_name + '\n'
                                            	                            	                            	                            	                            	                            	                + '商家用户：' + item.business_user + '\n'
                                            	                + '保质时间：' + item.shelf_life_time + '\n'
                                            	                            	                            	                            	                + '定制价格：' + item.customized_price + '\n'
                                            	                            	                            ,
